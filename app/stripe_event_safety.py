...
def _bounded_parse_webhook(payload: bytes, signature: str | None) -> dict[str, Any]:
    """Verify with Stripe first, then reject pathological audit identifiers.

    Stripe signatures remain the trust boundary. These bounds only prevent a
    valid-but-pathological event from persisting oversized identifiers into the
